...
namespace SerialPortTerminal
{
  ...
  public partial class frmTerminal2 : Form
  {
    ...
    */
    ...
    private void sendString(string strCommand, string strDescription)
    {
    ...
    #region Added Functions
        ...
        // Pass user information the Experiment form
        public String userName // retrieving a value from
        {
            get
            {
        ...
        public String userGender // retrieving a value from
        {
            get
            {
                return this.comboBoxGender.Text;
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            string dataPath = System.IO.Path.GetDirectoryName(Application.ExecutablePath).ToString() + "\\_data\\users\\";
            string userName = textBoxUserName.Text;
            string userGender = comboBoxGender.Text;
            //writeData(dataPath+ userName + "_" + userGender + ".txt", "Username:" + userName + " - Gender:" + userGender);
            writeData(dataPath + userName + "_" + userGender + ".txt", "--------------------------------");
            writeData(dataPath + userName + "_" + userGender + ".txt", "Time: "+ DateTime.Now.ToShortDateString() +" "+ DateTime.Now.ToShortTimeString() );



            //Open experiment window
            frmKeeponExperiment f3 = new frmKeeponExperiment(userName,userGender);
            //f3.TopMost = true;
            ...
            private void writeData(string filename, string text)
            {
                if (!File.Exists(filename))
                {
                    FileStream file = File.Create(filename);
                    file.Close();
                    File.AppendAllText(filename, "USER NAME: " + userName + " - GENDER: " + userGender + Environment.NewLine);
                }
                File.AppendAllText(filename, text + Environment.NewLine);
            }
        #endregion

    #region Two Keepons Movements
            // Movents of two Keepon, if modify, also modify in Termimal1
            public void KeeponRightSpeak() //Paula
            {
                sendString("MOVE PAN -20;", "Move Pan -20");
                Thread.Sleep(500);
                sendString("MOVE PON UP;", "Jump");
                Thread.Sleep(1000);
                sendString("MOVE PAN 0;", "Move Pan 0");
                Thread.Sleep(500);
            }

            public void KeeponLeftSpeak() //Paul
            {
                (this.Owner as frmTerminal1).sendString("MOVE PAN 20;", "Move Pan 20");
                Thread.Sleep(500);
                (this.Owner as frmTerminal1).sendString("MOVE PON UP;", "Jump");
                Thread.Sleep(1000);
                (this.Owner as frmTerminal1).sendString("MOVE PAN 0;", "Move Pan 0");
                Thread.Sleep(500);
            }

            public void KeeponRightChoice() //Paula
            {
                sendString("MOVE PON UP;", "Jump");
                //Thread.Sleep(1000);
            }

            public void KeeponLeftChoice() //Paul
            {
                (this.Owner as frmTerminal1).sendString("MOVE PON UP;", "Jump");
                //Thread.Sleep(1000);
            }
