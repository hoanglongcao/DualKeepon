...
excel_file = load_workbook(filename='Quiz_2_Pictures.xlsx')
...
def pictures(folderName,numberColumn):
    ...
    Arguments:
    - folderName for the name of the folder (in a string)
    - numberColumn for the gender (3 for left and 4 for right)
    ...
    stop = 0
    while stop == 0:
        a = str(sheet_quiz.cell(row = i+4,column = numberColumn).value)
        a = folderName + '/' + a
...
pictures_right = pictures("ImagesRight",4)
pictures_left = pictures("ImagesLeft",3)
