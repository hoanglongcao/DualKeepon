/* My Keepon Arduino controller
   ...
   KeeponÂ® is a trademark of BeatBots LLC.
   
   Minor modification by James Kennedy 09/08/2013
   
   Minor modification by Hoang-Long Cao 16/11/2015
*/
...
void bootup()
{
  digitalWrite(SDA, LOW);
  digitalWrite(SCL, LOW);
  Serial.println("Waiting for My Keepon...");
...
void loop() {
  ...
  while (analogRead(0) > 512) {
    //query();// Don't read motor and button states
    ...
    if (Serial.available() > 0) {
      ...
      if (parseMsg(msg, cmd, &device)) {
        int result = 1;
        int attempts = 0;
        while (result != 0 && attempts++ < 200) {
          Wire.beginTransmission(device);
          Wire.write((byte)cmd[0]);
          Wire.write((byte)cmd[1]);
          result = (int)Wire.endTransmission();
          if (result == 0) Serial.println("COMMAND SUCCEEDED ");
          if (attempts == 199) Serial.println("COMMAND FAILED ");
        }
      }
    }
    // Loop cycle
